...
histQ_SM = r.TH1D("LikeQ_SM", "SM like Q dist", 40, 15000, 22000)
histQ_NP = r.TH1D("LikeQ_NP", "NP like Q dist", 40, 15000, 22000)
...
def Q(Nsig):
	...
	for exp in range(Nexp):
		...
		for mtt in range(300,1000, 30):
			...
			SM_n_i = mttrandom_SM[exp].GetBinContent(mttrandom_SM[exp].FindFixBin(mtt)) # observed num
			NP_mu_i = mttmerge_NP.GetBinContent(mttmerge_NP.FindFixBin(mtt)) # expected num
			NP_n_i = mttrandom_NP[exp].GetBinContent(mttrandom_NP[exp].FindFixBin(mtt)) # observed num

			logL_SM = SM_n_i* log(SM_mu_i) - SM_mu_i + logL_SM
			logL_NP = NP_n_i* log(NP_mu_i) - NP_mu_i + logL_NP
			#print logL_SM, logL_NP
		likeQ.append(-2*(logL_NP-logL_SM))
	#print likeQ
	return likeQ
...
for exp in range(Nexp):
	histQ_SM.Fill(Q(30000)[exp])
	histQ_NP.Fill(Q(31000)[exp])
